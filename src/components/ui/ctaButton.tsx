  ...
  return (
    <Tag {...props} href={href} target={target} rel={rel} title={title} className={twMerge(parentWrapper, className)}>
      {btnStyle === "CTA_PRIMARY" ? (
        <span className="w-8 h-8 min-w-8 min-h-8 rounded-full bg-(--ssc-uk-main-white-color) text-(--ssc-uk-cta-button-background) flex items-center justify-center absolute top-1/2 -left-full -translate-y-1/2 group-hover/btn:left-1.5 transition-all duration-300">
      ...
      {btnStyle === "CTA_PRIMARY" ? (
        ...
        <span
          className={twMerge(
            ...
            theme === "DARK" ? "bg-(--ssc-uk-main-black-color)" : "bg-(--ssc-uk-main-white-color)",
          )}></span>
      )}
    </Tag>
  );
